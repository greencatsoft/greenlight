...
object OptionMatcher extends Matcher[Option[_], Be, Emptiness] {

  override def matches(actual: Option[_], expected: Emptiness): Unit = if (!actual.isEmpty)
    throw TestFailureException(s"Expected '$actual' to be empty.")

  override def notMatches(actual: Option[_], expected: Emptiness): Unit = if (actual.isEmpty)
